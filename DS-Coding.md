...
压缩路径一Find 操作，先找到根节点， 再将查找路径上所有结点都挂到根结点下
```c++
...
> 性质2证明：若红黑树总高度 = h ，则根节点黑高 $bh>=\frac{h}{2}$，因此内部结点数 $n>=2^{\frac{h}{2}}-1$，由此推出 $h<=2\log_2(n+1)$
...
基本操作：插入【注意溢出分裂】、删除【如果为终端结点，则直接删除；若不是终端结点，使用直接前驱或者直接后继取代被删除的元素】
